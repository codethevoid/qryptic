...
export const InviteEmail = (id: string, name: string) => (
  <Html>
    <Head />
    <Preview>You have been invited to join team {name} on Qryptic!</Preview>
    <Tailwind>
      <Body className="bg-white font-sans">
        <Container className="mx-auto my-8 w-full max-w-[400px] rounded-xl border border-solid border-zinc-200 px-6 py-10">
          ...
          <Text className="text-sm">
            You have been invited to join team {name} on Qryptic! You can view the invite by
            clicking the button below.
          </Text>
          <Section className="my-6 text-center">
            <Button
              className="block rounded-md bg-black py-2.5 text-center text-sm font-medium text-white no-underline"
              href={`${protocol}${appDomain}/invites/${id}`}
            >
              View invite
            </Button>
          </Section>
