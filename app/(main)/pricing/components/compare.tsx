...
import { CircleCheckFill } from "@/components/icons/circle-check-fill";
    ...
    <div className="px-4">
      <MaxWidthWrapper>
        ...
        <div className="sticky top-[48px] z-10 grid w-full grid-cols-[230px_1fr_1fr_1fr_1fr]">
          <div
            className={`compare-grid-item flex flex-col justify-center border border-b border-r-0 bg-background p-4 ${!inView ? "rounded-tl-none" : "rounded-tl-md"}`}
          >
          ...
          <div
            className={`compare-grid-item flex items-center border border-b border-r-0 bg-background p-4 max-[1100px]:flex-col max-[1100px]:space-y-2 min-[1100px]:justify-between ${!inView ? "rounded-tr-none" : "rounded-tr-md"}`}
          >
        ...
        <div className="grid w-full grid-cols-[230px_1fr_1fr_1fr_1fr] bg-background">
          ...
          <TableCell
            ...
            className="justify-between font-medium"
            info={
              "Implement granular access controls to ensure customized access and permissions for each user."
            }
          />
          {plans.map((plan: CustomPlan, i) => (
            <TableCell
          ...
          <TableCell
            value="Support level"
            className="justify-between rounded-bl-md font-medium"
            info={"The level of support you will receive."}
          />
          {plans.map((plan: CustomPlan, i) => (
            <TableCell key={plan.id} value={plan.supportLevel} />
          ))}
          <TableCell value="Dedicated" className="rounded-br-md" />
        </div>
      </MaxWidthWrapper>
    </div>
  );
